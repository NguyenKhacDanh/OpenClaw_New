...
export const channelsHandlers: GatewayRequestHandlers = {
  ...
  "channels.loginQrStart": async ({ req, params, respond, context }) => {
    console.log("[DEBUG loginQrStart] typeof params:", typeof params);
    console.log("[DEBUG loginQrStart] params JSON:", JSON.stringify(params));
    console.log("[DEBUG loginQrStart] req.params JSON:", JSON.stringify(req.params));
    const rawChannel = (params as { channel?: unknown }).channel;
    console.log("[DEBUG loginQrStart] rawChannel:", rawChannel);
    const channelId = typeof rawChannel === "string" ? normalizeChannelId(rawChannel) : null;
    console.log("[DEBUG loginQrStart] channelId:", channelId);
    if (!channelId) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "channels.loginQrStart requires a channel param"),
      );
      return;
    }
    const plugin = getChannelPlugin(channelId);
    if (!plugin?.gateway?.loginWithQrStart) {
      respond(
        ...
        errorShape(
          ErrorCodes.INVALID_REQUEST,
          `channel ${channelId} does not support QR login`,
        ),
      );
      return;
    }
  ...
  "channels.loginQrWait": async ({ params, respond, context }) => {
    const rawChannel = (params as { channel?: unknown }).channel;
    const channelId = typeof rawChannel === "string" ? normalizeChannelId(rawChannel) : null;
    if (!channelId) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "channels.loginQrWait requires a channel param"),
      );
      return;
    }
    const plugin = getChannelPlugin(channelId);
    if (!plugin?.gateway?.loginWithQrWait) {
      respond(
        ...
        errorShape(
          ErrorCodes.INVALID_REQUEST,
          `channel ${channelId} does not support QR login wait`,
        ),
      );
      return;
    }
