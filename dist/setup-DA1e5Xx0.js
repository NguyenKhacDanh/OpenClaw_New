...
import { g as readConfigFileSnapshot, x as writeConfigFile } from "./auth-profiles-CbvzvUuD.js";
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-BUJO_rNJ.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-z0wEetrr.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-CqctVEda.js");
		const { logConfigUpdated } = await import("./logging-BvNXT9PB.js");
	...
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-BeUG2MBc.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-DkrKEaQr.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-DOk9PWXP.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-BQZYvGBq.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-BuWQ09xx.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { setupChannels } = await import("./onboard-channels-BILfWA4X.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-CR8nUXPn.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-Blhy9uyS.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-BLoT_z_G.js");
