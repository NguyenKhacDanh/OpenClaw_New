	...
	register(api) {
		...
		api.registerCommand({
			...
			handler: async (ctx) => {
				...
				if (action === "disarm") {
					if (ctx.channel === "webchat" && !ctx.gatewayClientScopes?.includes("operator.admin")) return { text: "⚠️ /phone disarm requires operator.admin." };
				...
				if (action === "arm") {
					if (ctx.channel === "webchat" && !ctx.gatewayClientScopes?.includes("operator.admin")) return { text: "⚠️ /phone arm requires operator.admin." };
