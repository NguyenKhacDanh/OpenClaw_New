...
import { v as resolveUserPath } from "./utils-ChfYK_zu.js";
import { g as readConfigFileSnapshot, x as writeConfigFile } from "./auth-profiles-Bx_pc0K9.js";
import { u as resolveGatewayPort } from "./paths-DnMZ9c-4.js";
import { l as normalizeSecretInputString } from "./types.secrets-D6PgnNgv.js";
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-BvjWIyNP.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-C1c-sFaJ.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-gzI0W96h.js");
		const { logConfigUpdated } = await import("./logging-DqiIgJ-9.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-Cd-j4vZl.js");
	let nextConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
	const { ensureAuthProfileStore } = await import("./agents/auth-profiles.runtime.js");
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-BgGtnDFb.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-CrYSOysr.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-CKzqXFm3.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-Dog3JvDw.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-Dk6bkU-A.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-f6HD1_H2.js");
		const { setupChannels } = await import("./onboard-channels-C4FoKyPj.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-ReDR-WPd.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-Cw9j4Z3D.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-C5OqyfLe.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-BuFRDVrh.js");
